...
class TransactionServiceTest {
        ...
        void getAll_delegatesToRepository() {
                ...
                when(transactionRepository.findAll(Mockito.<Specification<Transaction>>any(), eq(pageable)))
                                .thenReturn(expected);
        ...
        @Test
        void create_transfer_passesComputedDailyTotalToRules() {
                TransactionCreateRequest request = new TransactionCreateRequest(
                ...
                verify(rules).validateTransfer(
                                eq(request), eq(fromAccount), eq(toAccount), eq(customerUser),
        ...
        @Test
        void create_transfer_propagatesRulesException() {
                TransactionCreateRequest request = new TransactionCreateRequest(
                                FROM_IBAN, TO_IBAN, null, new BigDecimal("100.00"), TransactionType.TRANSFER, null);
        ...
        void create_deposit_happyPath_creditsToAccount() {
                ...
                assertNotNull(result);
                verify(rules).validateDeposit(request, toAccount);
                verify(accountRepository).save(toAccount);
                verify(rules, never()).validateTransfer(any(), any(), any(), any(), any());
        }
        ...
        void create_withdrawal_happyPath_debitsFromAccount() {
                ...
                assertNotNull(result);
                verify(rules).validateWithdrawal(
        ...
        @Test
        void create_unsupportedType_delegatesToRules() {
                TransactionCreateRequest request = new TransactionCreateRequest(
                                null, null, null, new BigDecimal("50.00"), null, null);

                doThrow(new BadRequestException("Unsupported transaction type: null"))
                                .when(rules).rejectUnsupportedType(null);

                assertThrows(BadRequestException.class,
                                () -> transactionService.create(request, customerUser));

                verify(rules).rejectUnsupportedType(null);
