...
public record CreateAccountsRequest(
		...
		@NotNull(message = "minimumAllowedBalance is required") @DecimalMax(value = "0", inclusive = true, message = "minimumAllowedBalance must be zero or negative (overdraft floor)") @Digits(integer = 12, fraction = 2, message = "minimumAllowedBalance has too many digits") BigDecimal minimumAllowedBalance) {
}
